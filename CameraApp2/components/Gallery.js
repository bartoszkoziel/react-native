...
export default function Welcome(props) {
  ...
  const handleImgView = (uri, id, height, width) => {
    console.log("chosen pic: ", id);
    props.navigation.navigate("imgview", {id: id, height: height, width: width, uri: uri, refresh: getAssets})
  }
  ...
  const getAssets = async () => {
    try {
      ...
      const map = []
      assets.assets.forEach((e) => map.push({ id: e.id, uri: e.uri, filename: e.filename, height: e.height, width: e.width }))
      console.log(map)
      setAssets(map)
    } catch (e) {
      console.error(e)
